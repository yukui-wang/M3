//
//  CMPSelectContactViewController.m
//  M3
//
...
@interface CMPSelectContactViewController ()<UITableViewDelegate, UITableViewDataSource, UISearchResultsUpdating,UISearchControllerDelegate, CMPForwardSearchDelegate> {
    
    CMPSelectContactView *_listView;
...
@property (nonatomic, retain) CMPForwardSearchViewController *searchResultVC;
...
static CMPSelectContactViewController *curVC = nil;
@implementation CMPSelectContactViewController
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    //     SOSwizzlefDifferentClassInstanceMethod([RCForwardManager class], self, @selector(forwardWithConversationType:targetId:content:isCombine:), @selector(cmp_forwardWithConversationType:targetId:content:isCombine:));
...
+ (void)setCurVC:(CMPSelectContactViewController*)vc {
...
+ (BOOL)handleUCStartChatPage:(NSString*)chatId bGroup:(BOOL)group chatTitle:(NSString*)title {
    ...
    if (bForwarding) {
        ...
        if (group) {
            object.subtype = CMPRCConversationType_GROUP;
        }
        else {
            object.subtype = CMPRCConversationType_PRIVATE;
        }
        [curVC showForwardView:object];
    }
...
- (void)viewDidLoad {
    ...
    [CMPSelectContactViewController setCurVC:self];
    [CMPSelectContactViewController setForwarding:YES] ;
    [self setTitle:SY_STRING(@"forward_sendto")];
    [self createBackButton];
    if (!self.shareToUcDic && self.filePaths.count<=0 && !self.filePath) {
        if (![self.msgModel.content isKindOfClass:CMPBusinessCardMessage.class]) {//卡片消息
            [self createMultipleSelectButton];//多选按钮
        }
    }
    [self createSubView];
...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(messageBaseCellUpdateSendingStatus:) name:KNotificationMessageBaseCellUpdateSendingStatus object:nil];
}
...
- (void)mutipleSelectButtonAction:(id)sender {
    NSLog(@"mutipleSelectButtonAction");
    CMPSelectMultipleContactViewController *multipleSelectVC = [[CMPSelectMultipleContactViewController alloc] init];
    multipleSelectVC.msgModel = _msgModel;
    multipleSelectVC.targetId = _targetId;
    multipleSelectVC.forwardSource = _forwardSource;
    multipleSelectVC.selectedMessages = _selectedMessages;
    multipleSelectVC.forwardSucess = self.forwardSucess;
    [self.navigationController pushViewController:multipleSelectVC animated:YES];
    
//    if (INTERFACE_IS_PAD) {
//        CMPNavigationController *nav = [CMPNavigationController.alloc initWithRootViewController:multipleSelectVC];
...
//多选按钮
- (void)createMultipleSelectButton {
    self.bannerNavigationBar.rightMargin = 10.f;
    NSString *backTitle = SY_STRING(@"picture_multi_select_btn_title");
    CGFloat backW = [backTitle sizeWithFontSize:[UIFont systemFontOfSize:16.f] defaultSize:CGSizeMake(MAXFLOAT, 0)].width;
    UIButton *returnButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [returnButton setFrame:CGRectMake(0, 0, backW, 44)];
    [returnButton setTitle:backTitle forState:UIControlStateNormal];
    ...
    [returnButton setTitleColor:[CMPThemeManager sharedManager].iconColor forState:UIControlStateNormal];
    [returnButton addTarget:self action:@selector(mutipleSelectButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [returnButton.titleLabel setFont:[UIFont systemFontOfSize:16.f]];
    self.bannerNavigationBar.rightBarButtonItems = @[returnButton];
}


- (void)createSubView {
    _listView = (CMPSelectContactView *)self.mainView;
...
//    self.searchViewController.searchBar.hidden = YES;
...
- (void)reloadContactData {
    ...
    __weak CMPSelectContactViewController *weakSelf = self;
...
- (void)enterSelAllContactVC {
    
    NSString *href = @"http://uc.v5.cmp/v1.0.0/html/ucStartChatPage.html";
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    return section == 1 ?_dataList.count:2;
}
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return section == 0 ? 0.f : 40.f;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    if (indexPath.section == 0) {
        ...
        cell.infoLabel.text = indexPath.row == 0 ?SY_STRING(@"forward_create_new_chat") :SY_STRING(@"forward_choose_group");
        if (indexPath.row == 1) {
...
- (void)showForwardView:(CMPMessageObject*)object {
    ...
    if (self.shareToUcDic) {
        ...
        //        id messageCategory = self.shareToUcDic[@"params"][@"messageCategory"];
        //        NSString *messageCategoryStr;
        //        if ([messageCategory isKindOfClass:NSString.class]) {
        //            messageCategoryStr = messageCategory;
        //        }else {
        //            messageCategoryStr = [NSString stringWithFormat:@"%ld",[messageCategory longValue]];
        //        }
        //
        ...
        //        CMPAppList_2 *appInfo = [appListModel appInfoWithType:@"default" ID:messageCategoryStr];
        
        //        content = [NSString stringWithFormat:@"[%@] %@",appInfo.appName,title];
        content = title;
