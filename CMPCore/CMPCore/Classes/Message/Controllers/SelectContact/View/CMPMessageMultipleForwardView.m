//
//  CMPMessageMultipleForwardView.m
//  M3
//
...
@implementation CMPMessageMultipleForwardView{
...
- (void)createSubViewIsShowImgView:(BOOL)isShowImgView {
    ...
    if(self.targetArr.count>5){
        _bgView.cmp_height = isShowImgView? kViewBigH + 50.f: kViewSmallH + 50.f;
        _bgView.cmp_centerY = self.height/2.f;
    }
...
//        _titleLab.text = @"分别发送给";
        _titleLab = [[UILabel alloc] initWithFrame:CGRectMake(x, y, 200, height)];
        _titleLab.text = SY_STRING(@"send_separately_to");//分别发送给
    ...
    //头像
    NSInteger totalCount = _targetArr.count;
    CGFloat margin = 10;
    NSInteger rowNum = 5;
    CGFloat w = ((310-40) - 4*margin)/rowNum;
        
    for(int i=0;i<totalCount;i++){
        CGFloat x1 = x + (i%rowNum) * (w + margin);
        CGFloat y1 = y + (i/rowNum) * (w + margin);
                
        UIImageView *iconView = [[UIImageView alloc] init];
        iconView.backgroundColor = UIColor.greenColor;
        iconView.frame = CGRectMake(x1, y1, w, w);
        iconView.layer.cornerRadius = w/2;
        iconView.layer.masksToBounds = YES;
        [_bgView addSubview:iconView];
        
        NSDictionary *dict = _targetArr[i];
        NSInteger type = [dict[@"type"] integerValue];
        if(type == 102){//文件助手
            iconView.image = [UIImage imageNamed:@"msg_file_assistant_icon"];
        }else{
            BOOL isGroup = [dict[@"subType"] integerValue] == 3;
            NSString *cid = dict[@"cid"];
            NSString *imageUrl = [CMPCore memberIconUrlWithId:cid];
            if(isGroup){
                imageUrl = [CMPCore rcGroupIconUrlWithGroupId:cid];
            }
            [iconView sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"guesture.bundle/ic_def_person.png"]];
        }
        
    }
    
    y = totalCount>rowNum? y+2*(w+margin) : y+(w+margin);
...
- (BOOL)isContentFieldEditing{
    return _contentField.isEditing;
}
