...
static CGFloat const kTextFieldH = 18.f;

@interface CMPAssociateAccountEditNewView()<UITextFieldDelegate,CMPLoginViewTextFieldDelegate>
...
@property (strong, nonatomic) CMPSeparateImgButton *scanButton;
...
@implementation CMPAssociateAccountEditNewView
...
- (void)tapScanButton:(UIButton *)btn {
...
- (void)updateConstraints {
    ...
    [self.hostView mas_updateConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo(@(kTextFieldH));
    }];
    
    [self.hostUnderline mas_updateConstraints:^(MASConstraintMaker *make) {
        make.leading.equalTo(self).inset(kViewMargin);
        make.trailing.equalTo(self);
    ...
    NSString *scanTitle = SY_STRING(@"login_scan_button");
    CGFloat scanBtnW = [scanTitle sizeWithFontSize:[UIFont systemFontOfSize:14.f] defaultSize:CGSizeMake(MAXFLOAT, 0)].width + 20.f;
    [_scanButton mas_updateConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(scanBtnW);
        make.height.equalTo(@20.f);
        make.top.equalTo(self.noteUnderline.mas_bottom).inset(14.f);
        make.trailing.equalTo(self).inset(14.f);
    }];
    
    [self.saveButton mas_updateConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_scanButton.mas_bottom).offset(30);
        if (@available(iOS 11.0,*)) {
//            make.bottom.equalTo(self.mas_safeAreaLayoutGuideBottom).inset(16 + self->_keyboardHeight);
...
//            make.bottom.equalTo(self.mas_bottom).inset(kCommonInset + self->_keyboardHeight);
            make.trailing.equalTo(self.mas_trailing).inset(kViewMargin);
        }
        make.height.equalTo(@42);
        if (!self.deleteButton.hidden) {
            make.leading.equalTo(self.mas_centerX).inset(7.5);
        } else {
            make.leading.equalTo(self.mas_leading).inset(kViewMargin);
        }
    }];
...
//            make.bottom.equalTo(self.mas_bottom).inset(kViewMargin + self->_keyboardHeight);
            make.leading.equalTo(self.mas_leading).inset(kViewMargin);
        }
        make.height.equalTo(@42);
        if (!self.deleteButton.hidden) {
...
- (UITextField *)hostView {
    if (!_hostView) {
        ...
        _hostView.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SY_STRING(@"login_server_host1") attributes:self.attributesDic];
        _hostView.tag = kHostViewTag;
        
        [self setLeftView:_hostView];
    }
    return _hostView;
}
...
- (UITextField *)portView {
    if (!_portView) {
        ...
        _portView.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SY_STRING(@"login_server_port1") attributes:self.attributesDic];
        _portView.keyboardType = UIKeyboardTypeNumberPad;
        _portView.tag = kPortViewTag;
        [self setLeftView:_portView];
    }
    return _portView;
}
...
- (UITextField *)usernameView {
    if (!_usernameView) {
        ...
        [self setLeftView:_usernameView];
    }
...
- (CMPLoginViewTextField *)passwordView {
    if (!_passwordView) {
        ...
        [self setLeftView:_passwordView];
    }
...
- (UITextField *)noteView {
    if (!_noteView) {
        ...
        _noteView.attributedPlaceholder = [[NSAttributedString alloc] initWithString:SY_STRING(@"login_server_note1") attributes:self.attributesDic];
...
- (UIButton *)saveButton {
    if (!_saveButton) {
        ...
        image = [[CMPThemeManager sharedManager] skinColorImageWithImage:image];
        ...
        highlightedImage = [[CMPThemeManager sharedManager] skinColorImageWithImage:highlightedImage];
...
- (CMPSeparateImgButton *)scanButton {
    if (!_scanButton) {
        _scanButton = [[CMPSeparateImgButton alloc] init];
        [_scanButton addTarget:self action:@selector(tapScanButton:) forControlEvents:UIControlEventTouchUpInside];
        NSString *scanTitle = SY_STRING(@"login_scan_button");
        [_scanButton setTitle:scanTitle forState:UIControlStateNormal];
        [_scanButton setTitleColor:[UIColor cmp_colorWithName:@"theme-fc"] forState:UIControlStateNormal];
        _scanButton.titleLabel.font = [UIFont systemFontOfSize:14.f];
        [_scanButton setImage:[[CMPThemeManager sharedManager] skinColorImageWithName:@"login_view_scan_qrcode_blue_icon"] forState:UIControlStateNormal];
    }
    return _scanButton;
}
...
- (void)setLeftView:(UITextField *)tf {
    UIView *leftView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 14.f, tf.height)];
    UILabel *leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 8.f, 14.f, tf.height - 10.f)];
    leftLabel.textAlignment = NSTextAlignmentCenter;
    leftLabel.textColor = [UIColor cmp_colorWithName:@"hl-fc3"];
    leftLabel.text = @"*";
