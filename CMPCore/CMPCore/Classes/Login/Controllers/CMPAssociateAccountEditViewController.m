...
@interface CMPAssociateAccountEditViewController ()<SyScanViewControllerDelegate, CMPDataProviderDelegate>
{
    void(^loginAssAccountRslt)(NSInteger state);
}
@property (strong, nonatomic) CMPAssociateAccountEditView *editView;
...
@implementation CMPAssociateAccountEditViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _editView = (CMPAssociateAccountEditView *)self.mainView;
    [self setupMainView];
    if(_editMode) {
...
- (void)setupMainView {
    ...
    //ks fix -- V5-46408【M3兼容】ipad 7.0sp3 设置关联账号页面，关联键和设置vpn按钮重叠
    [self.vpnEnterView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(14);
        make.right.mas_equalTo(-14);
        make.bottom.mas_equalTo(self.editView.saveButton.mas_top).offset(-30);
...
//    if ([self.loginDBProvider findServerWithUniqueID:newServer.uniqueID]) {
//        // 关联了已经存在的服务器
//        if (_editMode && [newServer.uniqueID isEqualToString:_accountModel.serverUniqueID]) {
//            // 编辑模式，没有改动服务器地址，不报错
//        } else {
//            [self hideLoadingViewWithoutCount];
//            [self showAlertMessage:SY_STRING(@"ass_add_err1")];
//            return;
//        }
//    }
...
- (void)addScanButton {
    if (!_editMode) {
        [_editView showScanButtonWithAction:^{
            [self showScan];
        }];
    }
}
