...
/// Control the display position of the PopupController
typedef NS_ENUM(NSUInteger, zhPopupPosition) {
    zhPopupPositionNone = 0,//默认为不设置
    zhPopupPositionMiddle,//如果设置了。则开启
    zhPopupPositionTop,
};
...
@property (nonatomic,copy) void(^PositionChangedBlock)(CGFloat frameY);

- (instancetype)init NS_UNAVAILABLE;
+ (instancetype)new NS_UNAVAILABLE;

@property (nonatomic, assign) CGFloat smallHeight;
@property (nonatomic, assign) CGFloat initTopY;//初始展示的topY，需要大于0
@property (nonatomic, assign) CGFloat midPositionY;//初始中间状态Y值
@property (nonatomic, assign) CGFloat topPositionY;//顶部状态Y值
@property (nonatomic, assign) zhPopupPosition popupPosition;
