...
//    CGRect screenBound = [[UIScreen mainScreen] bounds];
    ...
    _nonWebViewHeight = self.webView.frame.origin.y;
    //screenBound.size.width-self.transitionView.frame.size.width + screenBound.size.height-self.transitionView.frame.size.height;
}
...
//    UIImageWriteToSavedPhotosAlbum(_screenShotImageView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        ...
        [UIView animateWithDuration:duration
                         ...
                         completion:^(BOOL finished) {
                             if ([self.viewController isKindOfClass:[CMPBaseWebViewController class]]) {
                                 [((CMPBaseWebViewController *)self.viewController) reLayoutSubViews];
                             }
                         }];
    }
...
- (void) performFlipTransition {
    ...
    if ([direction isEqualToString:@"right"]) {
        ...
        NSDictionary *errorDic= [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:40001],@"code",@"direction should be one of up|down|left|right",@"message",@"",@"detail", nil];
...
- (void) performCurlTransition {
    ...
    if ([direction isEqualToString:@"up"]) {
        ...
        NSDictionary *errorDic= [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:40002],@"code",@"direction should be one of up|down",@"message",@"",@"detail", nil];
        CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsDictionary:errorDic];
        [self.commandDelegate sendPluginResult:pluginResult callbackId:_command.callbackId];
        return;
    }
...
- (BOOL) loadHrefIfPassed:(NSString*) href
{
    ...
    if ([href containsString:@"cmp_orientation=auto"]) {
        bannerVc.allowRotation = YES;
    } else {
        bannerVc.allowRotation = NO;
    }
    if ([href containsString:@"useNativebanner=1"]) {
        [bannerVc showNavBarforWebView:@1];
    } else if ([href containsString:@"useNativebanner=0"]) {
        [bannerVc showNavBarforWebView:@0];
    }
...
//            NSString *urlParams = nil;
          ...
          /*  if (![bareFileName hasSuffix:@".html"]) {
            ...
            NSURL *url;
            NSURL *origUrl = self.wkWebView.URL;;
            // edit by guoyl for bug  at 2016/6/24
            if ([href hasPrefix:@"file"] || [href hasPrefix:@"http"]) {
...
//                    NSString *replaceWith = [@"/" stringByAppendingString:bareFileName];
                    filePath = [origUrl.absoluteString stringByReplacingOccurrencesOfString:origUrl.path withString:bareFileName];
           ...
           /* if (urlParams != nil) {
            ...
            NSDictionary *errorDic= [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInteger:40003],@"code",@"href must be null, a .html file or a #navigationhash",@"message",@"",@"detail", nil];
