...
/// Return YES if Denied 返回YES  拒绝 授权
...
- (void)getPostImageWithAlbumModel:(TZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        //过滤后的相册显示 列表缩略图
        TZAssetModel *tz = model.models.lastObject;
        id asset = tz.asset;
        if (!self.sortAscendingByModificationDate) {
            tz = model.models.firstObject;
            asset = tz.asset;
        }
        if (!asset) {
...
- (PHImageRequestID)getPhotoTemPathWithAsset:(id)asset targetSize:(CGSize)size completion:(void (^)(NSString *, NSDictionary *, BOOL isDegraded))completion {
    
    CGFloat photoWidth = TZScreenWidth;
    if (photoWidth > _photoPreviewMaxWidth) {
        photoWidth = _photoPreviewMaxWidth;
    }
    if ([asset isKindOfClass:[PHAsset class]]) {
        if (@available(iOS 11, *)) {
            PHAsset *phAsset = (PHAsset *)asset;
            if (phAsset.playbackStyle == PHAssetPlaybackStyleImageAnimated) {
                //动图单独处理
                return [self getAnimatedImgDataWithAsset:phAsset completion:^(NSString *photoPath, NSDictionary *info) {
                    if (completion) {
                        completion(photoPath,info,NO);
                    }
                }];
            }
        } else {
...
//        PHAsset *phAsset = (PHAsset *)asset;
//
//        if (phAsset.pixelWidth <size.width*TZScreenScale) {
//            imageSize = CGSizeMake(phAsset.pixelWidth, phAsset.pixelHeight);
//        }
//        else {
//            CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
//            imageSize = CGSizeMake(size.width*TZScreenScale, size.width *aspectRatio*TZScreenScale);
//        }
        if (photoWidth < TZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
            imageSize = AssetGridThumbnailSize;
        } else {
        ...
        PHImageRequestID imageRequestID = [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:imageSize contentMode:PHImageContentModeAspectFill options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            ...
            BOOL isDegrad = [[info objectForKey:PHImageResultIsDegradedKey] boolValue];
            if (downloadFinined && result &&!isDegrad) {
                result = [self fixOrientation:result];
                NSString *path = [self writeToTempImagePath:result];
                if (completion) completion(path,info,isDegrad);
            }
            // Download image from iCloud / 从iCloud下载图片
            if ([info objectForKey:PHImageResultIsInCloudKey] && !result) {
                PHImageRequestOptions *option = [[PHImageRequestOptions alloc]init];
                ...
                [[PHImageManager defaultManager] requestImageDataForAsset:asset options:option resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                    ...
                    if (!isDegrad) {
                        ...
                        if (resultImage) {
                            resultImage = [self fixOrientation:resultImage];
                            NSString *path = [self writeToTempImagePath:resultImage];
                            if (completion) completion(path,info,isDegrad);
                        }
                    }
                }];
        ...
        if (photoWidth == TZScreenWidth || photoWidth == _photoPreviewMaxWidth) {
            dispatch_async(dispatch_get_global_queue(0,0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    NSString *path = [self writeToTempImagePath:fullScrennImage];
                    if (completion) completion(path,nil,NO);
                });
            });
        }
    }
...
/// Get Original Photo / 获取原图
- (void)getOriginalPhotoPathWithAsset:(id)asset completion:(void (^)(NSString *photoPath,NSDictionary *info))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
      if (@available(iOS 11, *)) {
          PHAsset *phAsset = (PHAsset *)asset;
          if (phAsset.playbackStyle == PHAssetPlaybackStyleImageAnimated) {
              //动图单独处理
              [self getAnimatedImgDataWithAsset:phAsset completion:completion];
          }
      } else {}
        
        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
            if (orientation == UIImageOrientationUp) {
                // iOS 11中，iPhone 7及以上设备，系统用了新的格式来存储视频和图片HEIF，必须转换一下
                UIImage *image = [UIImage imageWithData:imageData];
                imageData = UIImageJPEGRepresentation(image, 0.75);
                NSString *path = [self tempImagePath];
                [imageData writeToFile:path atomically:YES];
                if (completion) completion(path,info);
            }
            else {
                UIImage *image = [UIImage imageWithData:imageData];
        ...
        dispatch_async(dispatch_get_global_queue(0,0), ^{
            CGImageRef originalImageRef = [assetRep fullResolutionImage];
            UIImage *originalImage = [UIImage imageWithCGImage:originalImageRef scale:1.0 orientation:UIImageOrientationUp];
            NSString *path = [self writeToTempImagePath:originalImage];
...
- (NSString *)tempImagePath {
    return [self tempImagePath:@"png"];
}
- (NSString *)tempImagePath:(NSString *)extension {
    NSString *rootPath = [CMPFileManager uploadTempFilePath];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyyMMddHHmmssSSS"];
    NSDate *date = [NSDate date];
    NSString *dateStr = [dateFormatter stringFromDate:date];
    NSString *filePath = [NSString stringWithFormat:@"%@/图片%@.%@",rootPath,dateStr,extension] ;
    dateFormatter = nil;
    return filePath;
}
...
{
    NSString *filePath = [self tempImagePath];
...
//获取动图
- (PHImageRequestID)getAnimatedImgDataWithAsset:(PHAsset *)asset completion:(void (^)(NSString *photoPath,NSDictionary *info))completion{
   return  [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
       NSString *imagePath = [self tempImagePath:@"gif"];
       [imageData writeToFile:imagePath atomically:YES];
       if (completion) {
           completion(imagePath,info);
       }
   }];
}
