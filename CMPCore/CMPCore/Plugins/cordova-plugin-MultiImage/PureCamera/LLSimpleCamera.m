...
-(void)capture:(void (^)(LLSimpleCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error))onCapture exactSeenImage:(BOOL)exactSeenImage animationBlock:(void (^)(AVCaptureVideoPreviewLayer *))animationBlock
{
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
        ...
        NSMutableDictionary *metadataiInfo = [NSMutableDictionary dictionary];
        [metadataiInfo setObject:metadata forKey:(__bridge NSString *)kCGImagePropertyExifDictionary];
        
        NSMutableDictionary *info = [NSMutableDictionary dictionary];
        [info setObject:[metadataiInfo copy] forKey:UIImagePickerControllerMediaMetadata];
        [info setObject:image forKey:UIImagePickerControllerOriginalImage];
        [info setObject:(__bridge NSString *)kUTTypeImage forKey:UIImagePickerControllerMediaType];
        
        // trigger the block
        if(onCapture) {
            dispatch_async(dispatch_get_main_queue(), ^{
                onCapture(self, image, [info copy], error);
            });
        }
    }];
}
