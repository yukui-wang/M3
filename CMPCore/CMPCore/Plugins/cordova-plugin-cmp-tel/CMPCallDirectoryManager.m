...
NSString * const CMPCallDirectoryManagerAppGroupFileName = @"CallDirectoryData";

@interface CMPCallDirectoryManager ()
...
@implementation CMPCallDirectoryManager
...
- (void)reload:(void (^)(NSError *error))completion {
    if (![self writeDataToAppGroupFile]) {
        NSError *error = [NSError errorWithDomain:@"写文件失败" code:-1 userInfo:nil];
        completion(error);
        return;
    }
...
- (void)clearAllData:(void (^)(NSError *))completion {
...
- (BOOL)writeDataToAppGroupFile {
    ...
    NSURL *containerURL = [fileManager containerURLForSecurityApplicationGroupIdentifier:self.groupIdentifier];
    containerURL = [containerURL URLByAppendingPathComponent:CMPCallDirectoryManagerAppGroupFileName];
