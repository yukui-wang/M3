...
+ (instancetype)sharedRCIM {
    ...
    dispatch_once(&onceToken, ^{
        if (__rongUIKit == nil) {
            ...
            __rongUIKit.enableMessageRecall = YES;
            __rongUIKit.enableMessageMentioned = YES;
            __rongUIKit.maxRecallDuration = 120;
            __rongUIKit.maxReadRequestDuration = 120;
            __rongUIKit.hasNotifydExtensionModuleUserId = NO;
            __rongUIKit.enabledReadReceiptConversationTypeList =
            ...
            __rongUIKit.GIFMsgAutoDownloadSize = 1024;
            __rongUIKit.enableSendCombineMessage = NO;
            __rongUIKit.enableBurnMessage = NO;
            __rongUIKit.enableTypingStatus = YES;
            __rongUIKit.enableSyncReadStatus = YES;
            __rongUIKit.showUnkownMessage = YES;
            __rongUIKit.reeditDuration = 300;
            __rongUIKit.sightRecordMaxDuration = 10;
            __rongUIKit.enableMessageReference = YES;
        }
    });
    return __rongUIKit;
}
...
- (void)postLocalNotificationIfNeed:(RCMessage *)message {
    ...
    if ([RCIMClient sharedRCIMClient].sdkRunningMode == RCSDKRunningMode_Background) {
        if (message.content.mentionedInfo.isMentionedMe) {
            ...
            if (!self.disableMessageNotificaiton && ![self checkNoficationQuietStatus]) {
                if (message.conversationType == ConversationType_Encrypted) {
                    [[RCLocalNotification defaultCenter]
                        postLocalNotification:NSLocalizedStringFromTable(@"receive_new_message", @"RongCloudKit", nil)
                                     userInfo:dictionary];
                } else {
...
#pragma clang diagnostic pop
    if (0 == nLeft && [RCIMClient sharedRCIMClient].sdkRunningMode == RCSDKRunningMode_Foreground &&
        !self.disableMessageAlertSound && ![self checkNoficationQuietStatus] && isCustomMessageAlert) {
        //获取接受到会话
...
//                        }
                        ...
                        BOOL playSound = YES;
                        if ([self.receiveMessageDelegate respondsToSelector:@selector(needPlaySystemSound:)]) {
                            playSound = [self.receiveMessageDelegate needPlaySystemSound:message];
                        }
                        if (playSound) {
                            [[RCSystemSoundPlayer defaultPlayer]
                             playSoundByMessage:message completeBlock:^(BOOL complete) {
    ...
    if (nLeft == 0 && isCustomMessageAlert) {
        ...
        if (ConversationType_CHATROOM == message.conversationType)
            return;
        // add by zl 20170911 群系统消息不做本地通知
...
- (void)setExclusiveSoundPlayer {
    if (self.isExclusiveSoundPlayer) {
