...
- (void)startDownload:(RCMessageModel *)messageModel {
    ...
    if (messageModel.messageDirection == MessageDirection_SEND && ![imageContent.imageUrl hasPrefix:@"http"]) {
        ...
        if (image) {
            ...
            UILabel *failLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.view.bounds.size.width / 2 - 75,
                                                                           self.view.bounds.size.height / 2, 150, 30)];
            ...
            success:^(NSString *mediaPath) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (image) {
                        ...
                        image = [RCKitUtility imageNamed:@"exclamation" ofBundle:@"RongCloud.bundle"];
                        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
                        [imageView setCenter:CGPointMake(blockSelf.view.bounds.size.width / 2,
                                                         blockSelf.view.bounds.size.height / 2 - 50)];
                        [blockSelf.view addSubview:imageView];
                        UILabel *failLabel =
                            [[UILabel alloc] initWithFrame:CGRectMake(blockSelf.view.bounds.size.width / 2 - 75,
                                                                      blockSelf.view.bounds.size.height / 2, 150, 30)];
