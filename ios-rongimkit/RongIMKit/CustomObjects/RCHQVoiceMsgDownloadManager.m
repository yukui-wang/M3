...
- (void)networkChanged:(NSNotification *)note {
    RCReachability *curReachable = [note object];
    RCNetworkStatus status;
    if (![curReachable respondsToSelector:@selector(currentReachabilityStatus)]) {
        RCLogV(@"networkChanged: unknown reachabily obj");
        status = RC_ReachableViaWiFi;
    } else {
        status = [curReachable currentReachabilityStatus];
    }
    self.status = status;
