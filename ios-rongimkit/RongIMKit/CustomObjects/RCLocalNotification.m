...
- (void)postLocalNotification:(NSString *)formatMessage userInfo:(NSDictionary *)userInfo {
    ...
    if ([RCIMClient sharedRCIMClient].pushProfile.isShowPushContent) {
        ...
        _localNotification.alertBody = NSLocalizedStringFromTable(@"receive_new_message", @"RongCloudKit", nil);
    }
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    if ([RCIM sharedRCIM].showUnkownMessageNotificaiton && message.objectName && !message.content) {
#pragma clang diagnostic pop
    ...
    if ((ConversationType_GROUP == message.conversationType)) {
        ...
        if ([RCIM sharedRCIM].publicServiceInfoDataSource) {
            ...
            serviceProfile =
                [[RCIMClient sharedRCIMClient] getPublicServiceProfile:(RCPublicServiceType)message.conversationType
