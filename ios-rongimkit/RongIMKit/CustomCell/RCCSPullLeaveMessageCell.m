...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.contentLabel.textColor = RCDYCOLOR(0xffffff, 0x707070);
        self.contentLabel.layer.masksToBounds = YES;
        self.contentLabel.layer.cornerRadius = 5.f;
        self.contentLabel.backgroundColor = RCDYCOLOR(0xc9c9c9, 0x232323);
...
- (void)setDataModel:(RCMessageModel *)model {
    ...
    CGSize __textSize = [RCKitUtility getTextDrawingSize:text
                                                    font:[UIFont systemFontOfSize:14.0f]
                                         constrainedSize:CGSizeMake(maxMessageLabelWidth, MAXFLOAT)];
    __textSize = CGSizeMake(ceilf(__textSize.width), ceilf(__textSize.height));
    CGSize __labelSize = CGSizeMake(__textSize.width + 10, __textSize.height + 6);
    self.contentLabel.frame = CGRectMake((self.baseContentView.bounds.size.width - __labelSize.width) / 2.0f - 5, 0,
                                         __labelSize.width + 10, __labelSize.height);

    NSTextCheckingResult *textCheckingResult =
        [NSTextCheckingResult linkCheckingResultWithRange:[text rangeOfString:@"留言"]
