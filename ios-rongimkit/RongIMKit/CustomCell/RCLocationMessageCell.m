...
- (void)setDestructViewLayout {
    RCLocationMessage *_locationMessage = (RCLocationMessage *)self.model.content;
    if (_locationMessage.destructDuration > 0) {
        ...
        if (self.messageDirection == MessageDirection_RECEIVE) {
            self.destructView.frame = CGRectMake(CGRectGetMaxX(self.pictureView.frame) + 4.5,
                                                 CGRectGetMaxY(self.pictureView.frame) - 13 - 8.5, 21, 12);
        } else {
            self.destructView.frame = CGRectMake(CGRectGetMinX(self.pictureView.frame) - 25.5,
                                                 CGRectGetMaxY(self.pictureView.frame) - 13 - 8.5, 21, 12);
        }
    } else {
