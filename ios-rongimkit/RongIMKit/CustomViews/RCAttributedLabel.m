...
- (void)generateAttributedString {
    self.attributedText = [self generateAttributedString:self.originalString];
}

- (NSMutableAttributedString *)generateAttributedString:(NSString *)str {
    if (!str) {
        return nil;
    }
            ...
            if (attributeDictionary) {
                if (str.length >= (range.location + range.length)) {
                    NSAttributedString *subString =
                        [[NSAttributedString alloc] initWithString:[str substringWithRange:range]
...
- (NSUInteger)characterIndexAtPoint:(CGPoint)p {
    ...
    /**
     iOS 字符串长度计算遇到 " \r\n"的时候，字符串长度计算有问题。所以替换掉
     */
    NSString *str = [self.originalString copy];
    ...
    NSMutableAttributedString *optimizedAttributedText = [self generateAttributedString:resultStr];
