...
- (void)fetchJSBridgeCommandsFromJsWithID:(NSString *)ID {
    __weak CDVCommandQueue* weakSelf = self;
    NSString* js = [NSString stringWithFormat:@"__CMPBridgeNativeGetParam__('%@')", ID];
    ...
    [_viewController.webViewEngine evaluateJavaScript:js
                                    completionHandler:^(id obj, NSError* error) {
                                        if ((error == nil) && [obj isKindOfClass:[NSString class]]) {
                                            ...
                                            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:[queuedCommandsJSON dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingAllowFragments error:nil];
                                            NSString *plugin = dic[@"plugin"];
                                            NSString *action = dic[@"action"];
                                            NSDictionary *param = [dic[@"param"] cdv_JSONObject];
                                            NSString *callbackID = dic[@"id"];
                                            
                                            if (!plugin || !action || !param || !callbackID) {
                                                NSLog(@"zl---[%s]参数错误:%@", __FUNCTION__, dic);
                                                return;
                                            }
                                            
                                            NSMutableArray *array = [NSMutableArray arrayWithObjects:@[callbackID, plugin, action, @[param], @"1"], nil];
                                            NSMutableArray* commandBatchHolder = [[NSMutableArray alloc] init];
                                            [_queue addObject:commandBatchHolder];
                                            [commandBatchHolder addObject:array];
                                            [weakSelf executePending];
                                        }
                                    }];
}
...
- (BOOL)execute:(CDVInvokedUrlCommand*)command
{
    ...
    if (command.isJSBridge) {
        obj.commandDelegate = _viewController.jsBridgeCommandDelegate;
    } else {
        obj.commandDelegate = _viewController.commandDelegate;
    }
    ...
    NSLog(@"ks log --- CDVCommandQueue execute : %@.%@",command.className,methodName);
