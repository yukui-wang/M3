...
- (id)initWithString:(NSString *)string {
    ...
    if (self = [super init]) {
        self.cellClass = @"XZQATextCell";
        self.ideltifier = @"XZQAModelideltifier";
...
- (void)setupWithStr:(NSString *)qaStr {
    ...
    CGFloat width = self.scellWidth-56;
    CGSize s = [self.attrString.string sizeWithFontSize:font defaultSize:CGSizeMake(width, MAXFLOAT)];
    NSInteger height = s.height+1;
    self.contentSize = CGSizeMake(s.width, height);
    
}

- (CGFloat)cellHeight{
    return self.contentSize.height+20+28;
}
...
//    NSString *pattern = @"\\[([^\\]]*?)\\]\\(file:([^\\)]*)[\\)]?\\)";
    ...
    if (results.count == 0) {
        XZQATextModel *model = [[XZQATextModel alloc] initWithString:qaStr];
    ...
    [results enumerateObjectsUsingBlock:^(NSTextCheckingResult*  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (idx == 0) {
            if (range.location == 0) {
                ...
                XZQATextModel *model = [[XZQATextModel alloc] initWithString:string];
        ...
        XZQAFileModel *model = [[XZQAFileModel alloc] initWithFileName:fileName fileJson:fileJson];
        [array addObject:model];
    }];
