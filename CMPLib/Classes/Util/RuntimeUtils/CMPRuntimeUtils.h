 ...
 * this macro has unstablility sometimes when system version changes, so use method + [CMPRuntimeUtils callWithTarget:selector:argumemts:returnValue:] below
 */
//example: id a = call(self, @selector(test:intValue:floatValue:size:), UIEdgeInsetsMake(10, 10, 100, 1), 100, 900.8f, CGSizeMake(10, 10));

@interface CMPRuntimeUtils : NSObject
