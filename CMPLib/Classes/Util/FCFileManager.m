...
+ (NSString *)copyFileToTempWithPath:(NSString *)path {
    NSString *newPath = [path stringByRemovingPercentEncoding];
    //解决文件名带有%时，无法转换path而造成文件读取失败并且最终发送文件失败的问题
    if (newPath) {
        path = newPath;
    }
    ...
    return [self copyFile:path toPath:tempPath];
    
}

+ (NSString *)copyFileToMyFileWithPath:(NSString *)path {
    NSString *newPath = [path stringByRemovingPercentEncoding];
    //解决文件名带有%时，无法转换path而造成文件读取失败并且最终发送文件失败的问题
    if (newPath) {
        path = newPath;
    }
    NSString *docPath = [CMPFileManager localSavedFilePath];
    
    NSString *pathExtension = path.lastPathComponent;
    docPath = [docPath stringByAppendingPathComponent:pathExtension];
    return [self copyFile:path toPath:docPath];
    
}

+ (NSString *)copyFile:(NSString *)origPath toPath:(NSString *)destPath {
    origPath = [origPath stringByRemovingPercentEncoding];
    destPath =  [destPath stringByRemovingPercentEncoding];
    if (!origPath || origPath.length == 0) {
        NSLog(@"原始路径不能为空");
        return nil;
    }
    ...
    if ([origPath isEqualToString:destPath]) {
        return origPath;
    }
//    if (![[NSFileManager defaultManager] fileExistsAtPath:origPath]) {
//        NSLog(@"复制文件失败--源文件不存在:%@",origPath);
//        return nil;
//    }
    NSError *err = nil;
    NSString *title = origPath.lastPathComponent;
    NSFileManager *fileMgr = [NSFileManager defaultManager];
    if ([fileMgr fileExistsAtPath:destPath]) {
        [fileMgr removeItemAtPath:destPath error:&err];
    }
    [fileMgr copyItemAtPath:origPath toPath:destPath error:&err];
    if (err) {
        if (err.code == 516) {
            return destPath;
        }
...
#pragma mark 移动文件
+ (NSString *)moveFileToTempWithPath:(NSString *)path {
    NSString *newPath = [path stringByRemovingPercentEncoding];
    //解决文件名带有%时，无法转换path而造成文件读取失败并且最终发送文件失败的问题
    if (newPath) {
        path = newPath;
    ...
    NSString *pathExtension = path.lastPathComponent;
    tempPath = [tempPath stringByAppendingPathComponent:pathExtension];
    return [self moveFile:path toPath:tempPath];
}

+ (NSString *)moveFileToMyFileWithPath:(NSString *)path {
    NSString *newPath = [path stringByRemovingPercentEncoding];
    //解决文件名带有%时，无法转换path而造成文件读取失败并且最终发送文件失败的问题
    if (newPath) {
        path = newPath;
    ...
    NSString *pathExtension = path.lastPathComponent;
    docPath = [docPath stringByAppendingPathComponent:pathExtension];
    return [self moveFile:path toPath:docPath];
}

+ (NSString *)moveFile:(NSString *)origPath toPath:(NSString *)destPath {
    if (!origPath || origPath.length == 0) {
        NSLog(@"原始路径不能为空");
        return nil;
    }
    ...
    [fileMgr moveItemAtPath:origPath toPath:destPath error:&err];
    if (err) {
        NSLog(@"移动文件失败--%@",title);
        switch (err.code) {
            case 516:
