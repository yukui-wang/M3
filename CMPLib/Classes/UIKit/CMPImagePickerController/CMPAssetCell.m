...
@implementation CMPAssetCell

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reload:) name:@"CMP_PHOTO_PICKER_RELOAD_NOTIFICATION" object:nil];
    return self;
}
...
- (void)setModel:(CMPAssetModel *)model {
    ...
    int32_t imageRequestID = [[CMPImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.CMP_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[CMPImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(CMPAssetCellType)type {
    _type = type;
    if (type == CMPAssetCellTypePhoto || type == CMPAssetCellTypeLivePhoto || (type == CMPAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == CMPAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.CMP_left = self.videoImgView.CMP_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == CMPAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.CMP_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:CMPOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[CMPImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        [self hideProgressView];
...
- (void)reload:(NSNotification *)noti {
    CMPImagePickerController *CMPImagePickerVc = (CMPImagePickerController *)noti.object;
    ...
    if (parentViewController.navigationController != CMPImagePickerVc) {
        return;
    }
    
    if (self.model.isSelected && CMPImagePickerVc.showSelectedIndex) {
        self.index = [CMPImagePickerVc.selectedAssetIds indexOfObject:self.model.asset.localIdentifier] + 1;
    }
    self.indexLabel.hidden = !self.selectPhotoButton.isSelected;
    if (CMPImagePickerVc.selectedModels.count >= CMPImagePickerVc.maxImagesCount && CMPImagePickerVc.showPhotoCannotSelectLayer && !self.model.isSelected) {
        self.cannotSelectLayerButton.backgroundColor = CMPImagePickerVc.cannotSelectLayerColor;
...
- (UIImageView *)videoImgView {
    if (_videoImgView == nil) {
        ...
        [videoImgView setImage:[UIImage CMP_imageNamedFromMyBundle:@"VideoSendIcon"]];
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.CMP_width - 44, 0, 44, 44);
    } else {
        _selectPhotoButton.frame = self.bounds;
    }
    _selectImageView.frame = CGRectMake(self.CMP_width - 27, 3, 24, 24);
    ...
    _imageView.frame = CGRectMake(0, 0, self.CMP_width, self.CMP_height);
    
    static CGFloat progressWH = 20;
    CGFloat progressXY = (self.CMP_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    _bottomView.frame = CGRectMake(0, self.CMP_height - 17, self.CMP_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.CMP_right, 0, self.CMP_width - self.videoImgView.CMP_right - 5, 17);
...
@interface CMPAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation CMPAlbumCell
...
- (void)setModel:(CMPAlbumModel *)model {
    _model = model;
    ...
    [[CMPImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.CMP_width - 24, 23, 24, 24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80, (self.CMP_height - titleHeight) / 2, self.CMP_width - 80 - 50, titleHeight);
...
@implementation CMPAssetCameraCell
