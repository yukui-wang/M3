...
@implementation CMPImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.CMP_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[CMPImageManager manager] fixOrientation:cropedImage];
...
@implementation UIImage (CMPGif)

+ (UIImage *)sd_CMP_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [CMPImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
