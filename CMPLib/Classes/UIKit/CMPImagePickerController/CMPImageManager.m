...
@interface CMPImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation CMPImageManager

CGSize AssetGridThumbnailSize;
CGFloat CMPScreenWidth;
CGFloat CMPScreenScale;

static CMPImageManager *manager;
...
- (void)setPhotoWidth:(CGFloat)photoWidth {
    ...
    CMPScreenWidth = photoWidth / 2;
}

- (void)setColumnNumber:(NSInteger)columnNumber {
    [self configCMPScreenWidth];

    _columnNumber = columnNumber;
    CGFloat margin = 4;
    CGFloat itemWH = (CMPScreenWidth - 2 * margin - 4) / columnNumber - margin;
    AssetGridThumbnailSize = CGSizeMake(itemWH * CMPScreenScale, itemWH * CMPScreenScale);
}

- (void)configCMPScreenWidth {
    CMPScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    CMPScreenScale = 2.0;
    if (CMPScreenWidth > 700) {
        CMPScreenScale = 1.5;
    }
}
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(CMPAlbumModel *model))completion {
    __block CMPAlbumModel *model;
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<CMPAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<CMPAssetModel *> *))completion {
    CMPImagePickerConfig *config = [CMPImagePickerConfig sharedInstance];
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<CMPAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        CMPAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:allowPickingVideo allowPickingImage:allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(CMPAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (CMPAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    CMPAssetModel *model;
    CMPAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == CMPAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == CMPAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == CMPAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == CMPAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [CMPAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (CMPAssetModelMediaType)getAssetType:(PHAsset *)asset {
    CMPAssetModelMediaType type = CMPAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = CMPAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = CMPAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = CMPAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = CMPAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ...
        if (model.type != CMPAssetModelMediaTypeVideo) {
            ...
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
                if (model.type != CMPAssetModelMediaTypeVideo) dataLength += imageData.length;
                assetCount ++;
            ...
            [[PHImageManager defaultManager] requestAVAssetForVideo:model.asset options:videoOptions resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
                NSNumber *size = @(0);
                if ([asset isKindOfClass:[AVURLAsset class]]) {
                    AVURLAsset *avAsset = (AVURLAsset *)asset;
                    [avAsset.URL getResourceValue:&size forKey:NSURLFileSizeKey error:nil];
                    dataLength += size.integerValue;
                }
                assetCount ++;
                if (assetCount >= photos.count) {
                    NSString *bytes = [self getBytesFromDataLength:dataLength];
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = CMPScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth == -1) {//表示原图，尺寸无限制
        PHAsset *phAsset = (PHAsset *)asset;
        imageSize = CGSizeMake(phAsset.pixelWidth, phAsset.pixelHeight);
    }else if (photoWidth < CMPScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = AssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * CMPScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(CMPAlbumModel *)model completion:(void (^)(UIImage *))completion {
    ...
    return [[CMPImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)getVideoOutputPathWithAsset:(PHAsset *)asset success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    [self getVideoOutputPathWithAsset:asset presetName:AVAssetExportPreset960x540 success:success failure:failure];
}
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([CMPImagePickerConfig sharedInstance].needFixComposition) {
...
- (CMPAlbumModel *)modelWithResult:(PHFetchResult *)result name:(NSString *)name isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets {
    CMPAlbumModel *model = [[CMPAlbumModel alloc] init];
...
- (CMPAssetModel *)createModelWithAsset:(PHAsset *)asset {
    CMPAssetModelMediaType type = [[CMPImageManager manager] getAssetType:asset];
    NSString *timeLength = type == CMPAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[CMPImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    CMPAssetModel *model = [CMPAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//@implementation CMPSortDescriptor
//
//- (id)reversedSortDescriptor {
//    return [NSNumber numberWithBool:![CMPImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
