...
#define CMPMAXYEAR 2099
#define CMPMINYEAR 1927


@interface CMPDatePicker ()
<UIPickerViewDelegate,UIPickerViewDataSource>
{
    ...
    CMPAlertPublicBodyStyle _datePickerStyle;
...
- (instancetype _Nullable)initWithTitle:(nullable NSString *)title
    ...
    UIView *currentView = [CMPCustomAlertViewHelper currentViewController].view;
    ...
    switch (bodyStyle) {
        case CMPAlertStyleShowYearMonthDayHourMinuteSecond:
            ...
            _pickerHeight = 160;
            break;
        case CMPAlertStyleShowYearMonthDayHourMinute:
            _pickerAlertViewWidth = 300;
            _font = 15;
            _pickerHeight = 140;
            break;
            
        case CMPAlertStyleShowYearMonthDay:
            _pickerAlertViewWidth = 280;
            _font = 15;
            _pickerHeight = 120;
            break;
        case CMPAlertStyleShowYearMonth:
            _pickerAlertViewWidth = 280;
            _font = 15;
            _pickerHeight = 120;
            break;
        case CMPAlertStyleShowHourMinuteSecond:
            _pickerAlertViewWidth = 280;
            _font = 15;
            _pickerHeight = 120;
            break;
    ...
    if (mode == 0) {
        UIView *alert = [[UIView alloc] initWithFrame:CGRectMake(10, (CMPAlertScreenH-300)/2, _pickerAlertViewWidth , 300)];
        ...
        _pickerAlertViewWidth = CMPAlertScreenW;
        _pickerHeight = 160;
        UIView *alert = [[UIView alloc] initWithFrame:CGRectMake(0, CMPAlertScreenH, _pickerAlertViewWidth , 300)];
...
- (instancetype)initWithTitle:(NSString *)title initialTime:(NSTimeInterval)initialTime delegate:(id)delegate footStyle:(CMPAlertPublicFootStyle)footStyle bodyStyle:(CMPAlertPublicBodyStyle)bodyStyle mode:(NSInteger)mode cancelButtonTitle:(NSString *)cancelButtonTitle okButtonTitles:(NSString *)sureButtonTitles handler:(void (^)(NSInteger, id _Nullable))handler{
    
    CMPDatePicker *picker = [[CMPDatePicker alloc]initWithTitle:title delegate:delegate footStyle:footStyle bodyStyle:bodyStyle mode:mode cancelButtonTitle:cancelButtonTitle okButtonTitles:sureButtonTitles handler:handler];
...
- (NSArray *)getNumberOfRowsInComponent{
    ...
    if (_datePickerStyle !=CMPAlertStyleShowYearMonth
        || _datePickerStyle !=CMPAlertStyleShowHourMinuteSecond) {
...
//    [_buttionContainerView addConstraint:NSLayoutAttributeBottom equalTo:_pickerAlertView offset:0];
    ...
    if (_buttionList.count == 0) {
        ...
        if (footStyle == CMPAlertPublicFootStyleVertical) {//竖排
...
- (void)initData{
    ...
    for (NSInteger i = CMPMINYEAR; i <= CMPMAXYEAR; i++) {
...
//    date = [NSDate dateWithTimeIntervalSince1970:1609862400000/1000];
    NSString *initialTimeStr = [[NSNumber numberWithDouble:_initialTime] stringValue];
    if (initialTimeStr.length==13) {
        date = [NSDate dateWithTimeIntervalSince1970:_initialTime/1000];
    }
    if (initialTimeStr.length <= 12 && initialTimeStr.length>1) {
        date = [NSDate dateWithTimeIntervalSince1970:_initialTime];
    }
    [self daysOnYear:date.year month:date.month];
    
    _yearIndex = date.year - CMPMINYEAR;
    ...
    if (_datePickerStyle == CMPAlertStyleShowYearMonthDayHourMinuteSecond){
    ...
    }else if (_datePickerStyle == CMPAlertStyleShowYearMonthDayHourMinute){
        indexArray = @[@(_yearIndex),@(_monthIndex),@(_dayIndex),
...
- (void)setSheetFrame{
    ...
    if (_mode == 0) {
        ...
        [UIView animateWithDuration:0.62 animations:^{
            ...
            rect1.origin.y = CMPAlertScreenH - heigth;
...
- (void)show{
    ...
    _isModal = NO;
    UIWindow *keyWindows = [CMPCustomAlertViewHelper getWindow];
    ...
    if (_setFrame == 0) {
        ...
        if (_mode == 1) {
            [UIView animateWithDuration:0.62 animations:^{
                CGRect rect1 = self.pickerAlertView.frame;
                rect1.origin.y = CMPAlertScreenH - rect1.size.height;
                self.pickerAlertView.frame = rect1;
            }];
        }
    }
    ...
    switch (_datePickerStyle) {
        case CMPAlertStyleShowYearMonthDayHourMinuteSecond:
            [self getNowDate:[NSDate date:_defalutDateString format:CMPDateStyleYYYYMMDDHHMMSS] animated:NO];
            break;
        case CMPAlertStyleShowYearMonthDayHourMinute:
            [self getNowDate:[NSDate date:_defalutDateString format:CMPDateStyleYYYYMMDDHHMM] animated:NO];
            break;
        case CMPAlertStyleShowYearMonthDay:
            [self getNowDate:[NSDate date:_defalutDateString format:CMPDateStyleYYYYMMDD] animated:NO];
            break;
        case CMPAlertStyleShowYearMonth:
            [self getNowDate:[NSDate date:_defalutDateString format:CMPDateStyleYYYYMM] animated:NO];
            break;
        case CMPAlertStyleShowHourMinuteSecond:
            [self getNowDate:[NSDate date:_defalutDateString format:CMPDateStyleHHMMSS] animated:NO];
            break;
...
- (void)showOnViewController{
    ...
    [[CMPCustomAlertViewHelper currentViewController] presentViewController:conVC animated:NO completion:^{
        if (self-> _mode == 1) {
...
- (void)hiddenAlertView{
    ...
    if (_mode == 1) {
        [UIView animateWithDuration:0.62 animations:^{
            CGRect rect1 = self.pickerAlertView.frame;
            rect1.origin.y = CMPAlertScreenH;
            self.pickerAlertView.frame = rect1;
        } completion:^(BOOL finished) {
            if (self -> _isModal) {
                [[CMPCustomAlertViewHelper currentViewController] dismissViewControllerAnimated:YES completion:^{
