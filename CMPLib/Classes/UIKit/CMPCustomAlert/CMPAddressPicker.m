...
@interface CMPAddressPicker ()
<UIPickerViewDelegate,UIPickerViewDataSource>
{
    CMPAlertPublicBodyStyle _pickerStyle;
...
/** 选中的省 */
@property(nonatomic, strong) CMPProvinceModel *selectProvinceModel;
/** 选中的市 */
@property(nonatomic, strong) CMPCityModel *selectCityModel;
/** 选中的区 */
@property(nonatomic, strong) CMPAreaModel *selectAreaModel;
...
@implementation CMPAddressPicker
...
-(void)buttionClick:(UIButton *)btn{
    if (btn.tag == 101) {
        CMPResultModel *model = [CMPResultModel new];
        model.province = [self.selectProvinceModel copy];
        if (_pickerStyle == CMPAlertAddressPickerShowCity) {
            model.city = [self.selectCityModel copy];
        }else if (_pickerStyle == CMPAlertAddressPickerShowArea){
            model.city = [self.selectCityModel copy];
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    switch (_pickerStyle) {
        case CMPAlertAddressPickerShowProvince:{
                CMPProvinceModel *model = self.provinceList[row];
        ...
        case CMPAlertAddressPickerShowCity:{
            if (component == 0){
                ...
                CMPCityModel *model = self.cityList[row];
                title = [NSString stringWithFormat:@"%@",model.name];
            }
        }
            break;
        case CMPAlertAddressPickerShowArea:{
            if (component == 0){
                ...
                CMPAreaModel *model = self.areaList[row];
                title = [NSString stringWithFormat:@"%@",model.name];
            }
        }
            break;
...
- (void)setDefalutOnAddressPickerView:(CMPResultModel *)defalutModel{
    ...
    CMPProvinceModel *tempProinceModel = nil;
    CMPCityModel *tempCityModel = nil;
    CMPAreaModel *tempAreaModel = nil;
    if (_pickerStyle == CMPAlertAddressPickerShowProvince) {
       NSString *province = defalutModel.province.name;
        if (!province) {
            NSLog(@"没有传入默认的省份名称，匹配不到情况下，默认值为内部实现的默认值");
        }else{
            for (CMPProvinceModel *model in self.provinceList) {
    ...
    }else if (_pickerStyle == CMPAlertAddressPickerShowCity){
        ...
        if (!city) {
            ...
            for (CMPCityModel *model in self.cityList) {
        ...
        if (!area) {
            ...
            for (CMPAreaModel *model in self.areaList) {
...
- (NSArray *)loadLocationData{
    ...
    NSURL *url = [bundle URLForResource:@"CMPCustomAlertView" withExtension:@"bundle"];
    NSBundle *plistBundle = [NSBundle bundleWithURL:url];
    
    NSString *filePath = [plistBundle pathForResource:@"CMPCity" ofType:@"plist"];
...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in dataSource) {
        CMPProvinceModel *proviceModel = [[CMPProvinceModel alloc]init];
        ...
        for (NSDictionary *cityDic in citylist) {
            CMPCityModel *cityModel = [[CMPCityModel alloc]init];
            ...
            for (NSDictionary *areaDic in arealist) {
                CMPAreaModel *areaModel = [[CMPAreaModel alloc]init];
