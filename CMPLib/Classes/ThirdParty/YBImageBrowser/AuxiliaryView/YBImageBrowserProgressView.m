...
- (void)showLoading {
    ...
    if (![self.imageView.layer animationForKey:@"ra"]) {
...
- (void)showText:(NSString *)text click:(void(^)(void))click {
    ...
    if ([text isEqualToString:[YBIBCopywriter shareCopywriter].imageIsInvalid]) {
        NSTextAttachment *attachmen =[[NSTextAttachment alloc]init];
        attachmen.bounds = CGRectMake(0, 0, 50, 40);
        attachmen.image =  [YBIBFileManager getImageWithName:@"image_Is_Invalid"];
        NSAttributedString *str = [NSAttributedString attributedStringWithAttachment:attachmen];
...
- (UIImageView *)imageView {
    if (!_imageView) {
        UIImage *img = [UIImage imageNamed:@"loading_animate_icon"];
