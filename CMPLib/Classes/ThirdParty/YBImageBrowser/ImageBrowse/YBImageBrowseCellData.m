...
/* 是否保存到本地过 */
@property (assign, nonatomic) BOOL isSavedToLocal;
...
//    self->_horizontalfillType = YBImageBrowseFillTypeUnknown;
    self->_verticalfillType = YBImageBrowseFillTypeCompletely;
    self->_horizontalfillType = YBImageBrowseFillTypeCompletely;
...
- (void)saveImageToCocalWithImage:(YBImage *)image {
    self.isSavedToLocal = YES;
    NSString *url = self.url.absoluteString;
    NSString *sourceId = [CMPCommonTool getSourceIdWithUrl:url];
    [CMPCommonTool.sharedTool saveImageToLocalWithImage:image imageData:image.imageData imgName:self.imgName from:self.from fromType:self.fromType fileId:sourceId isShowSavedTips:NO];
}
...
- (void)SavePhoto {
    [self yb_browserSaveToPhotoAlbum];
}
...
- (void)loadLocalImage {
    ...
    if (self.loadLocalImageFinishBlock) {
        self.loadLocalImageFinishBlock(self.image);
        self.loadLocalImageFinishBlock = nil;
    }
...
//        CGFloat value = receivedSize * 1.0 / expectedSize ?: 0;
        self->_downloadProgress += ((arc4random() % 35) * 0.01);
        if (self->_downloadProgress >= 0.99) {
...
- (void)downloadThumbImage {
    if (!self.thumbUrl) return;
    [YBIBWebImageManager downloadImageWithURL:self.thumbUrl progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
      
    } success:^(UIImage * _Nullable image, NSData * _Nullable nsData, BOOL finished) {
        if (!finished) return;
        
        YBIB_GET_QUEUE_ASYNC(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            self.thumbImage = [YBImage imageWithData:nsData];
            YBIB_GET_QUEUE_MAIN_ASYNC(^{
                if (self.dataState != YBImageBrowseCellDataStateCompressImageReady && self.dataState != YBImageBrowseCellDataStateImageReady) {
                    self.dataState = YBImageBrowseCellDataStateThumbImageReady;
                }
                [YBIBWebImageManager storeImage:self.thumbImage imageData:nsData forKey:self.thumbUrl toDisk:YES];
            });
        });
