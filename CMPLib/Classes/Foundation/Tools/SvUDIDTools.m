...
//static const char kKeyChainUDIDAccessGroup_IPhone[] = kM3PrefixAppIDInHouse;
//static const char kKeyChainUDIDAccessGroup_IPad[] = kM3PrefixAppIDInHouse;

@implementation SvUDIDTools

+ (NSString *)prefixAppID
{
    return @"LMTR39G5GC.com.seeyon.m3.appstore.new.phone"; //默认M3 AppStore版本
}
...
{
    if (!UDID) {
        UDID = [[SvUDIDTools _UDID_iOS7] copy];
    }
    return UDID;
}
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        accessGroup = [SvUDIDTools prefixAppID];//[NSString stringWithUTF8String:kKeyChainUDIDAccessGroup_IPad];
    }
    else {
        accessGroup = [SvUDIDTools prefixAppID];//[NSString stringWithUTF8String:kKeyChainUDIDAccessGroup_IPhone];
    }
    if (accessGroup != nil)
    {
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    [dictForQuery setValue:(id)kSecAttrAccessibleWhenUnlockedThisDeviceOnly forKey:(id)kSecAttrAccessible];
